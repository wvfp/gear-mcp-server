...
Herkömmliche KI-Assistenten können GDScript schreiben, arbeiten dabei aber im Wesentlichen blind. Sie generieren Code auf Basis von Trainingsdaten und hoffen, dass er funktioniert. **Gear ändert alles:**
...
Anstatt Werkzeuge für jede Godot-Klasse hardzukodieren, bietet Gear **generische Werkzeuge** (`add_node`, `create_resource`), die mit JEDER ClassDB-Klasse funktionieren, plus **ClassDB-Introspektionswerkzeuge**, mit denen die KI Klassen, Eigenschaften und Methoden dynamisch entdecken kann.
...
> **Designphilosophie**: Anstatt 90+ spezialisierte Werkzeuge (`create_camera`, `create_light`, `create_physics_material`) bereitzustellen, verwendet Gear generische `add_node`- und `create_resource`-Werkzeuge, die mit JEDER Godot-Klasse funktionieren. Die KI nutzt `query_classes`, um verfügbare Typen zu entdecken, und `query_class_info`, um ihre Eigenschaften zu erlernen — genau wie ein Entwickler, der die Godot-Dokumentation verwendet.
...
Jede Operation in Gear beinhaltet:
...
Gear verwendet eine Hybrid-Architektur:
...
- **npm-Paket** — mit `npx Gear` oder `npm install -g Gear` installieren
